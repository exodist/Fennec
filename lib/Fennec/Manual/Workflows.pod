...
=head1 CUSTOM WORKFLOW SYNOPSYS
    ...
    # Export a function that builds an instance of the workflow
    build_with 'my_workflow';

    # Method that returns a list of TestSet objects.
    sub testsets {
        ...
        return @testsets;
    }

    # Override add_item() to do whatever you want with testsets and workflows
    # that are children of your workflow.
    sub add_item {
        my $self = shift;
        my ($item) = @_;
        ...
        if ( we_handle( $item )) {
            ...
            # Let super handle items we don't care about.
            $self->SUPER::add_item( $item )
        }
    }
...
You can access the added items via $self->_workflows() and $self->_testsets().
