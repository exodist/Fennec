...
withought any modification. It is also possible to tell Fennec to use the
...
configuration file. Most config options can be overiridden using environment
variables. See Chapter 5 - Configuration for more info.
...
=head2 CHAPTER 5.2 - CONFIGURING THE RUNNER
...
=head3 CHAPTER 5.2.1 - Runner Configuration Options
...
=head3 CHAPTER 5.2.2 - Overriding Runner Options on The Command-Line
...
variables, the script recognises the following variables:
...
system is the existing and growing veriety of ways to define groups and
...
=item share/vim/functions.vim

This file defines the RunFennecLine() function. This function runs the current
test group in the current file.

=item share/vim/vimrc
...
=head1 CHAPTER 7 - TEST::BUILDER INFORMATION

Fennec does not use or depend on Test::Builder. However it has a module that
...
=head2 CHAPTER 7.1 - USING TEST::BUILDER TOOLS
...
=head2 CHAPTER 7.2 - USING TEST::BUILDER INSTEAD OF FENNEC ASSERTS
...
=head1 CHAPTER 8 - ADVANCED TOPICS

=head2 CHAPTER 8.1 - OUTPUT HANDLERS
...
namespace. See Chapter 5 - Configuration for the proper options to use.

=head2 CHAPTER 8.2 - COLLECTORS

Fennec allows for forking and parrallelization, it needs a way to send results
...
Chapter 5 - Configuration for the proper options to use.
