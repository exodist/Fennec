...
config file and write tests that require less boilderplate.
...
manage a suite it is loaded only once, and it can parallelize running fo both
...
than an END block. This is not necessary when you use Fennec to manage an
entire suite instead of standalone files.
